    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="font-serif text-4xl font-semibold tracking-tight">Evergreen Note Management</h1>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-[var(--ink-soft)]">
            Use this area to manage reusable knowledge cards, reading notes, checklists, and reference snippets.
          </p>
        </div>
          ...
          <tbody>
            {notes.map((note) => (
              <tr key={note.id} className="border-t border-black/6">
                ...
                <td className="px-6 py-4 text-[var(--ink-soft)]">{note.noteType ?? "Knowledge Note"}</td>
                <td className="px-6 py-4 text-[var(--ink-soft)]">{note.featured ? "Yes" : "No"}</td>
                <td className="px-6 py-4 text-[var(--ink-soft)]">{formatDate(note.updatedAt, "yyyy-MM-dd HH:mm")}</td>
                <td className="px-6 py-4">
                  <Link href={`/admin/notes/${note.id}`} className="inline-flex items-center gap-2 font-semibold text-[var(--accent-strong)]">
